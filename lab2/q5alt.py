S_BOX = [
]

RCON = [0,1,2,4,8,16,32,64,128,27,54]

def sub(w): return [S_BOX[x] for x in w]

def expand(key):
    w = [list(key[i:i+4]) for i in range(0,24,4)]
    for i in range(6,52):
        t = w[i-1][:]
        if i % 6 == 0:
            t = sub(t[1:] + t[:1])
            t[0] ^= RCON[i//6]
        w.append([w[i-6][j] ^ t[j] for j in range(4)])
    return w

def gmul(a,b):
    r = 0
    for _ in range(8):
        if b & 1: r ^= a
        a = ((a << 1) ^ (0x1B if a & 0x80 else 0)) & 255
        b >>= 1
    return r

def rounds(s, w, rnd):
    s = [S_BOX[x] for x in s]
    s = [s[4*((c+r)%4)+r] for c in range(4) for r in range(4)]

    if rnd != 12:
        for c in range(4):
            a = s[4*c:4*c+4]
            s[4*c:4*c+4] = [
                gmul(a[0],2)^gmul(a[1],3)^a[2]^a[3],
                a[0]^gmul(a[1],2)^gmul(a[2],3)^a[3],
                a[0]^a[1]^gmul(a[2],2)^gmul(a[3],3),
                gmul(a[0],3)^a[1]^a[2]^gmul(a[3],2)]
    k = sum(w[4*rnd:4*rnd+4], [])
    return [a^b for a,b in zip(s,k)]

def show(s):
    print(" ".join(f"{x:02X}" for x in s))

message = b"Top Secret Data"
message += bytes([16-len(message)%16] * (16-len(message)%16))

key = bytes.fromhex(
    "FEDCBA9876543210"
    "FEDCBA9876543210"
    "FEDCBA9876543210"
)

w = expand(key)

print("Plaintext:", message.hex().upper())
print("Key:", key.hex().upper())

print("\nKEY EXPANSION")
for i,x in enumerate(w):
    print(f"W{i:02}:", "".join(f"{b:02X}" for b in x))

s = list(message)

print("\nINITIAL ROUND")
s = [a^b for a,b in zip(s, sum(w[:4], []))]
show(s)

for r in range(1,13):
    print(f"\nROUND {r}")
    s = rounds(s,w,r)
    show(s)

print("\nCIPHERTEXT:")
print("".join(f"{x:02X}" for x in s))
